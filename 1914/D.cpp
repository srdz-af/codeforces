...
int main(){
    ...
    while(t--){
        ...
        rep(i, 0, 3) sort(act[i], act[i]+n, greater<pair<ll, ll>>());
        ll ans = 0;

        rep(i, 0, 3){
            rep(j, 0, 3){
                rep(k, 0, 3){
                    if(act[0][i].S == act[1][j].S || act[0][i].S == act[2][k].S || act[1][j].S == act[2][k].S) continue;
                    ans = max(ans, act[0][i].F + act[1][j].F + act[2][k].F);
                }
            }
        }
